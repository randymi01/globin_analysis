...
for (i in 1:nrow(rnaseq_samples)) {
    ...
    dir_id <- paste("../04.Alignment/", sample_id, "_PolyA_1", "/02_featureCounts2/read_counts_s0_PE.txt", sep = "") # Possible input values for read counting include: read_counts_s0 (unstranded), read_counts_s1 (stranded) and read_counts_s2 (reversely stranded).
...
write.table(annot.rnaSeqCount.matrix, "skin_poly_a_counts.csv", quote = F, row.names = T, sep = ",")
